...
var play = () => {
  ...
  for(i in level_data.pipes){
    ...
    // Draw pipe body
    draw_sprite(18, level_data.pipes[i][0] * 32, pipes_state[i].y + 32);
    draw_sprite(19, level_data.pipes[i][0] * 32 + 32, pipes_state[i].y + 32);
    
    end_pipe = false;
    for(k = ~~(pipes_state[i].y / 32); k < 21; k++){
    ...
    // Draw pipe top
    draw_sprite(16, level_data.pipes[i][0] * 32, pipes_state[i].y);
    draw_sprite(17, level_data.pipes[i][0] * 32 + 32, pipes_state[i].y);
    ...
    if(level_data.pipes[i][1]){
      ...
      // Pipe body below low position (can be overwritten)
      end_pipe = false;
      for(k = level_data.pipes[i][1] + 1; k < 21; k++){
        if(k < 20 && !level_data.tiles[k][level_data.pipes[i][0]] && !level_data.tiles[k][level_data.pipes[i][0] + 1] && !end_pipe){
          draw_tile(18, level_data.pipes[i][0], k);
          draw_tile(19, level_data.pipes[i][0] + 1, k);
        }
